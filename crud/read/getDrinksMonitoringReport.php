...
$items = json_decode(dbConn("SELECT Product_No, Description,UnitCostPrice FROM Products LEFT JOIN Items_Bought ON Product_No = ProductNo WHERE Category = 'Drinks' /*AND Product_No = 'pdt001'*/", array(), 'select'))->message;
...
foreach($items as $v){
	...
	$bf_sql = "select Product_No AS value, Description AS label, UnitCostPrice AS rate, Category,UnitQty,
			...
			left join 
				(select ProductNo, sum(QtyRecieved) AS reqsTotal from purchasedetails join purchaseorder on purchaseNo = purchase_no WHERE requisitionType = 'Internal' AND /*Station = '$station'*/ year(DateRecieved) = $year AND month(DateRecieved) < $month group by ProductNo) AS `reqsqty` on((Product_No = reqsqty.ProductNo))) 
			left join 
	...
	$bf_dbConn_res = json_decode(dbConn($bf_sql, array(), 'select'))->message[0];
	array_push($itemsbf_qty, $bf_dbConn_res->finish);
	$bfamt = $bf_dbConn_res->finish * $costprice;//echo $bfamt."\n";
	...
	/*$items_obj->*/$particularsDetails = json_decode(dbConn("select date_format(orderdate, '%D') as dateData, ifnull(soldQty,0) as soldQty,ifnull(round(BoughtQty,0),'-') AS BoughtQty, ifnull(format(round(BoughtAmount,0),0),'-') AS BoughtAmount, ifnull(format(soldAmount,0),0) AS soldAmount,ifnull(offersQty,'-') as offersQty,ifnull(format(offersAmount,0),'-') AS offersAmount,ifnull(spoiltQty,'-') as spoiltQty,ifnull(format(spoiltAmount,0),'-') AS spoiltAmount
																from orders 
																left join (select orderno, sum(qty) as soldQty, sum(cost) AS soldAmount, date(orderdate) as SalesOrderDate from orderdetails JOIN orders on orderNo = order_No where productno = '$pdtno' and year(orderdate) = $year and month(orderdate) = $month  group by date(orderdate)) as salesinfo on date(orderdate) = salesinfo.SalesOrderDate 
																left join (select date(daterecieved) as PurchRecDate, sum(qtyrecieved) as BoughtQty, sum(qtyrecieved) * $costprice AS BoughtAmount from purchasedetails join purchaseorder on PurchaseNo = Purchase_No where productno = '$pdtno' and year(daterecieved) = $year and month(daterecieved) = $month and RequisitionType = 'Internal' group by date(daterecieved)) as purchinfo on date(orderdate) = purchinfo.PurchRecDate
	...
	$totals_reqs = json_decode(dbConn("select ifnull(round(sum(qtyrecieved),0),0) AS totalQtyBought, ifnull(format(if(isnull(sum(finalamount)),if(isnull(sum(purchaseamount)),sum(standardcost),sum(purchaseamount)),sum(finalamount)),0),0) AS totalAmountBought FROM PurchaseDetails JOIN PurchaseOrder ON PurchaseNo = Purchase_No WHERE year(DateRecieved) = $year AND month(DateRecieved) = $month AND ProductNo = '$pdtno' AND RequisitionType = 'External'", array(), 'select'))->message[0];
	array_push($itemsqtytotals_reqs, $totals_reqs->totalQtyBought + $bf_dbConn_res->finish);
	array_push($itemsamttotals_reqs, $totals_reqs->totalAmountBought );
	
	$totals_sales = json_decode(dbConn("select ifnull(sum(qty),0) AS totalQtySold, format(sum(cost),0) AS totalAmountSold FROM OrderDetails JOIN Orders ON OrderNo = Order_No WHERE year(OrderDate) = $year AND month(OrderDate) = $month AND ProductNO = '$pdtno'", array(), 'select'))->message[0];
	array_push($itemsqtytotals_sales, $totals_sales->totalQtySold);
	array_push($itemsamttotals_sales, $totals_sales->totalAmountSold);
	
	$totals_offers = json_decode(dbConn("select ifnull(sum(qty),0) AS totalQtyOffered, format(sum(qty) * $costprice,0) AS totalAmountOffered FROM OffersDetails JOIN Offers ON OffersID = Offers.ID WHERE year(DateOccurred) = $year AND month(DateOccurred) = $month AND ProductNO = '$pdtno'", array(), 'select'))->message[0];
	...
	array_push($itemsfinish_qty, ($totals_reqs->totalQtyBought + $bf_dbConn_res->finish) - ($totals_sales->totalQtySold + $totals_offers->totalQtyOffered + $totals_spoilt->totalQtySpoilt) );
}
//print_r($items_col);
